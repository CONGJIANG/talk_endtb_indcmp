...
**Approaches**: 
  ...
  - **Pooled-IPD “indirect” strategy**: Pool IPD from both trials and estimate the indirect contrast between treatments via the common comparator (standard care).
:::

. . .

::: {style="font-size: 85%; background-color: #e6f7ff; padding: 10px; border-left: 5px solid #0077b6;"}
**In the following**, we will introduce four methodological strategies for data fusion in this cross-trial setting.
:::
...
- **External transportability**  
  Prior work studies how to transport causal effects from multiple randomized trials to a **new external target population** where experimental data may not be available (Dahabreh et al., 2020, 2024; Cole & Stuart, 2010; Westreich et al., 2017).

- **Common trial structure assumption**  
  Much of the causal transportability and IPD meta-analysis literature considers settings where trials evaluate the **same treatment set or comparator**, which simplifies pooling or transport of treatment effects (e.g., Dahabreh et al., 2020).

- **Our setting and contribution**  
  We define an **internal target population** as the **union of participants from endTB and TB-PRACTECAL** and focus on cross-trial causal comparison. The trials are **disconnected**, with **country- and trial-specific standards of care** and no guaranteed shared comparator. We compare four data-fusion strategies by combining ideas from transportability, target trial emulation (Hernán & Robins, 2016), and IPD meta-analysis (Riley et al., 2010).
:::
...
**Indices and Populations**: 
  ...
  - $\Omega_s \subset \Omega$: subpopulation enrolled in trial $S=s$
:::
...
**Limitation**: Assumes negligible differences in standard-care efficacy between endTB and TB-PRACTECAL


This directly reflects $\mathbb{E}[Y(c_1) - Y(c_2)] = 0$

:::


## (1B) Target Trial Emulation Approach
...
- Adjust for baseline covariates to control confounding.
...
::: {style="font-size: 85%;"}
2.  <span style="background-color:#FFF2CC">Extending inferences from trial participants to a target population (i.e., $\Omega$).<span>
:::

. . .

::: {style="font-size: 85%;"}
Due to $\Omega = \Omega_{\text{TB-PRACTECAL}} \cup \Omega_{\text{endTB}},$ we use both
...
part of the trial-eligible population.
:::

## Method: Direct and Indirect Method

::: {style="font-size: 85%;"}
<span style="background-color:#FFF2CC">In contrast to the trial emulation strategy, both the Direct and Indirect methods leverage randomization: the within trial randomization implies the conditional ramdomizaiton.<span>
:::
...
<span style="background-color:#FFF2CC"> Fortunately, this is guaranteed by randomization within each trial, no matter how many $L$s we measured. </span>
:::

. . .

::: {style="font-size: 85%;"}
Otherwise, if we do not have within trial randomization, we need to measure all important patient characteristics, **treatment assignment is unrelated to what outcomes would have been under different treatments**.
...
::: {style="font-size: 85%;"}
We consider three classes of estimators for the Direct contrast: 1. **G-computation**; 2. **Inverse Probability Weighting (IPW)**; 3. **One-step / Doubly-Robust estimator** (Appendix)
...
To address their lack of robustness, we focus on a **one-step (doubly-robust) estimator** in the main analysis. This estimator:

* Remains consistent if **either** the outcome model **or** the inclusion model is correctly specified.
...
| Method                      | Uses randomization | Uses all arms | Intuitive to clinicians | Doubly robust | Statistically efficient | Requires transportability |
| --------------------------- | :----------------: | :-----------: | :---------------------: | :-----------: | :---------------------: | :-----------------------: |
| **1A: Pooled-IPD Direct**   |          ✔         |       ✔       |            ✔            |       ✔       |            ✔            |             ✔             |
| **1B: Pooled-IPD Indirect** |          ✔         |       ✔       |            △            |       ✔       |            ✔            |             ✔             |
| **2: Trial Emulation**      |          ✖         |       ✖       |            ✔            |       △       |            △            |             ✔             |
:::
...
| 1A Traditional    | 1B Observational  | 2A Indirect       | 2B Direct         |
| ----------------- | ----------------- | ----------------- | ----------------- |
...
* **(1A) Traditional:** contrast of trial-specific treatment effects
  $\psi=\tau_a-\tau_b$

* **(1B) Observational:** contrast in the treat-only population
  $\psi=\tau'_a-\tau'_b$
...
* **1A & 2A:** exchangeability holds *within each trial*
* **1B:** exchangeability in the treat-only population
* **2B:** exchangeability in the pooled population $\Omega$
:::
...
* **1A:** requires a common control and equal contrasts across trials
* **1B:** requires equality of treatment effect in $\Omega_{trt}$ and $\Omega$
* **2A:** requires transportability for

  * controls (given country $V$), and
  * treatments (given $L$)
* **2B:** requires transportability of treatment effects given $L$
:::
...
| -------------------- | ------------------------------------------------------------------------------------------------ |
| **1A Traditional**   | Uses **fully pooled IPD from all arms**, maximizing effective sample size.           |
| **1B Observational** | Uses **all treated units** across trials but may discard controls for the main contrast.         |
| **2A Indirect**      | Uses **fully pooled IPD from all arms**, maximizing effective sample size.   |
| **2B Direct**        | Uses **fully pooled IPD from all arms**, maximizing effective sample size.                       |
:::
...
## Target Trial Emulation (Aim 1: PLHIV)  
