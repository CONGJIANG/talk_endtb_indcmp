...
**Approaches**: 
  ...
  - **Pooled-IPD “indirect” strategy**: Pool IPD from both trials and estimate the indirect contrast between treatments <span style="background-color:#FFF2CC">via the common comparator (standard care)<span>.
:::
...
::: {style="font-size: 85%;"}
- **Transportability**  
  Prior work studies how to transport causal effects from multiple randomized trials to a **new target population** where experimental data may not be available (Dahabreh et al., 2020, 2024; Cole & Stuart, 2010; Westreich et al., 2017). [@hernan2016] [@dahabreh2019extending]
:::
...
- **Our setting and contribution**  
  ...
  - We compare four strategies by combining ideas from *generalizability, transportability, target trial emulation, and IPD meta-analysis* (Hernán & Robins, 2016) (Riley et al., 2010).
  In particular, standard cares comparison can be solved in our framework by appropriately adjusting for differences.
:::
...
**Populations**: 
  ...
  - $\Omega_s \subset \Omega$: subpopulation of $\Omega$ enrolled in trial $S=s$
:::
...
|--------------|----------------------------------------------------|------------|
| **(1) Traditional Approach** | Compare each treatment v.s. its own control in each trial, then difference the effects | $\Omega$ (Pooled) |
...
## Method 3 and 4: Direct and Indirect Method
...
- Generalizability: extension of inferences from the trial to a target population that coincides, or is a subset of, the trial-eligible population [@hernan2016] [@dahabreh2019extending]
...
part of the trial-eligible population. [@dahabreh2019extending]
:::
...
<span style="background-color:#FFF2CC"> Fortunately, this is guaranteed by randomization within each trial, no matter how many covariates we measured. </span>
:::

. . .

::: {style="font-size: 85%;"}
Otherwise, if we do not have within trial randomization, we need to measure all important patient characteristics ($L$s), **treatment assignment is unrelated to what outcomes would have been under different treatments**.
...
\psi(a, b) &:=  \mathbb{E}[Y(a) - Y(b) \mid \Omega] \nonumber \\
    &= \mathbb{E}_{L\mid S=1}[\mathbb{E}(Y \mid A = a, L, S =1)] \mathbb{P}_{\Omega}(S=1) \\&\ \ \ \ - \mathbb{E}_{L\mid S=1}[\mathbb{E}(Y \mid A = b, L, S =2)] \mathbb{P}_{\Omega}(S=1)  \\ &\ \ \ \ + \mathbb{E}_{L\mid S=2}[\mathbb{E}(Y \mid A = a, L, S =1)] \mathbb{P}_{\Omega}(S=2) \nonumber \\ &\ \ \ \ - \mathbb{E}_{L\mid S=2}[\mathbb{E}(Y \mid A = b, L, S =2)] \mathbb{P}_{\Omega}(S=2),
\end{align}
...
| 1 Traditional    | 2 Observational  | 3 Indirect       | 4 Direct         |
| ----------------- | ----------------- | ----------------- | ----------------- |
...
* **(2) Observational:** contrast in the treat-only population.
...
* **1 & 2:** exchangeability holds *within each trial*
* **3:** exchangeability in the treat-only population
* **4:** exchangeability in the pooled population $\Omega$
:::
...
* **1:** requires a common control and equal contrasts across trials
